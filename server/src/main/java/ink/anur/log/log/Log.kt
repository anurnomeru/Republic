...
class Log {
    ...
    private val logger = Debugger(Log::class.java)
    ...
    private long load() throws IOException
    {
        ...
        if (file.isFile()) {
            ...
            if (!file.canRead()) {
                throw new LogException ("Fail to read slice log file or index file");
            }

            String filename = file . getName ();
            if (filename.endsWith(LogCommon.INSTANCE.getIndexFileSuffix())) {
                ...
                if (!logFile.exists()) {
                    logger.warn("Index file {} on gen {} was created, but did not create the slice log file, the index file will be deleted", filename, generation);
                    file.delete();
                    break;
                }
            } else if (filename.endsWith(LogCommon.Companion.getLogFileSuffix())) {
                long start = Long . parseLong (filename.substring(0, filename.length() - LogCommon.Companion.getLogFileSuffix().length()));
