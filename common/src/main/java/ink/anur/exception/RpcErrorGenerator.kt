...
class RpcErrorGenerator {
    companion object {
        ...
        init {
            ...
            RPC_ERROR_MAPPING["ERR2"] = { RpcUnderRequestException("在目标 Provider 根据指定接口找到了多个 RPC 实现类，请在 @KanashiRpcInject() 指定别名来对其中一个进行请求！") }
            RPC_ERROR_MAPPING["ERR3"] = { RpcUnderRequestException("在目标 Provider 无法根据指定的别名来找到对应的 RPC 实现，请确认 @KanashiRpcInject() 别名的正确性！") }
        }
    }
}