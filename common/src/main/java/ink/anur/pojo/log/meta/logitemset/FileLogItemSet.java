...
public class FileLogItemSet extends LogItemSet {
    ...
    public OffsetAndPosition searchFor(long targetOffset, int startingPosition) throws IOException {
        ...
        while (position + LogOverhead < size) {
            ...
            if (messageSize < AbstractStruct.Companion.getOriginMessageOverhead()) {
    ...
    public Iterator<LogItemAndOffset> iterator(int maxMessageSize) {
        return new IteratorTemplate<LogItemAndOffset>() {
            ...
            protected LogItemAndOffset makeNext() {
                ...
                if (size < AbstractStruct.Companion.getOriginMessageOverhead() || location + LogOverhead + size > end) { // 代表消息放不下了
                    return allDone();
                }
